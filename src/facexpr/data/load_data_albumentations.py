...
def make_dataloaders(
    ...
    train_alb = A.Compose([
        A.RandomResizedCrop(size=(img_size, img_size), scale=(
            0.8, 1.0), ratio=(0.9, 1.1), p=1.0),
        ...
        A.GaussNoise(sigma_limit=(10.0, 50.0), p=0.2),
        A.RandomFog(fog_coef=(0.1, 0.3), p=0.1),
        A.RandomSnow(p=0.1),
        A.CoarseDropout(
            min_holes=1, max_holes=1,
            min_height=int(img_size*0.2), max_height=int(img_size*0.2),
            min_width=int(img_size*0.2),   max_width=int(img_size*0.2),
            fill_value=0, p=0.2
        ),
